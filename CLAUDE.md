...
- **Gradient** (`backgroundGradientPreset` index into `GradientPresets.ALL`, 16 curated two-color combinations built from the existing `colors.xml` palette) — `setGradientBackground()` builds a `GradientDrawable` from the preset. `GradientSelectorDialog`/`GradientSelectorDialogPreference` mirror `ColorSelectorDialog`/`ColorSelectorDialogPreference`'s circular-grid picker structure (same layout/drawable reuse) but render each swatch as a two-color gradient and hand back a preset *index* instead of a resolved color.
- **Photo** (`backgroundPhotoUri`, a persisted content `Uri` string) — picked via `ActivityResultContracts.PickVisualMedia()` in `SettingPreferencesFragment` (no runtime permission needed), with `contentResolver.takePersistableUriPermission()` so the grant survives reboots. `setPhotoBackground()` decodes the bitmap and shows `scrimView` (a full-screen semi-transparent overlay added to `activity_motivation_locker.xml`, `GONE` in the other two modes) for text readability. If the URI can no longer be opened (e.g. the photo was deleted), it catches the failure and falls back to a plain white background rather than crashing — same defensive philosophy as `QuoteRepository`'s asset-loading fallbacks.

`backgroundTypeCategory`/`backgroundGradientCategory`/`backgroundPhotoPicker` in `pref.xml` are shown/hidden manually via `isVisible` in `SettingPreferencesFragment` (not XML `dependency`, since the controlling preference is a `ListPreference`, not boolean).
